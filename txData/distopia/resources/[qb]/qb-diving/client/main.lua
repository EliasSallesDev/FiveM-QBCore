    ...
    for k, v in pairs(Config.CoralLocations[CurrentDivingLocation.area].coords.Coral) do
        if Config.UseTarget then
            ...
            Zones[k]:onPlayerInOut(function(inside)
                if inside then
                    CurrentArea = k
                    exports['qb-core']:DrawText(Lang:t("info.collect_coral_dt"), 'left', 'qb-diving')
                else
                    CurrentArea = 0
                    exports['qb-core']:HideText('qb-diving')
                end
            end)
        end
    end
    ...
    for i = 1, #Config.SellLocations do
        ...
        if Config.UseTarget then
            ...
            zone:onPlayerInOut(function(inside)
                if inside then
                    inSellerZone = true
                    exports['qb-core']:DrawText(Lang:t("info.sell_coral_dt"), 'left', 'qb-diving')
                else
                    inSellerZone = false
                    exports['qb-core']:HideText('qb-diving')
                end
